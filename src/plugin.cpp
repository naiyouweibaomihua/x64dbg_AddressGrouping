...
DWORD WINAPI MsgLoopThread(LPVOID)
{
	...
	UpdateWindow(hwnd);
	refreshTreeView(); // 自动刷新一次，确保节点checkbox不失
				...
				if (ht.hItem)
				{
					...
					if (hParent) // 子节点
					{
						...
						if (sel == ID_MENU_ADD_COMMENT)
						{
							...
							if (Script::Gui::InputLine(u8"输入备注(输入删除)", comment))
							{
								std::string commentStr(comment); // 用构造函数
								commentStr = Utf8ToGbk(commentStr); // 转码
								if (strlen(comment) > 0)
	...
	ADD_GROUP_MENU:
		{
			...
			else if (sel == MENU_ADDGROUP_NEW)
			{
				...
				if (!groupName.empty())
				{
					groupName = Utf8ToGbk(groupName); // 转码
...
void insertAddressNode(const std::string& groupName, HTREEITEM hParent, duint addr, TVINSERTSTRUCTA& tvi)
{
	...
	if (it != addrComments.end() && !it->second.empty()) {
		...
		{
			display += " // ";
			display += Utf8ToGbk(std::string(comment)); // 转码，防止x64dbg注释中文乱码
		}
	}
	tvi.hParent = hParent;
