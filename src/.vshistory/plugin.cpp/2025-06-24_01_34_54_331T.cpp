    ...
    case WM_PAINT:
        {
            ...
            HBRUSH hBrush = CreateSolidBrush(RGB(200, 200, 200)); // 灰色背景
            ...
            {
                std::string groupTitle = "[" + group.first + "]";
                TextOutA(hdc, 10, y, groupTitle.c_str(), (int)groupTitle.length());
                y += 20;
                for (duint addr : group.second)
                {
                    char buf[64];
                    sprintf(buf, "  0x%llX", (unsigned long long)addr);
                    TextOutA(hdc, 20, y, buf, (int)strlen(buf));
                    y += 18;
                }
                y += 8;
            }
            if (groupMap.empty())
            {
                const char* tip = "(No key code addresses)";
                TextOutA(hdc, 10, y, tip, (int)strlen(tip));
            }
            EndPaint(hwnd, &ps);
            return 0;
        }
