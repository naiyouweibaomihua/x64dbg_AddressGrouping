    ...
    case WM_CREATE:
        {
            ...
            TreeView_SetExtendedStyle(hTree, TVS_EX_CHECKBOXES, TVS_EX_CHECKBOXES);
        ...
        if (((LPNMHDR)lParam)->hwndFrom == hTree)
        {
                ...
                if (ht.hItem)
                {
                    ...
                    if (hParent)
                    {
                        ...
                        if (sel == ID_MENU_ADD_COMMENT)
                        {
                            ...
                            if (Script::Gui::InputLine("输入备注(留空删除)", comment))
                            {
                                if (strlen(comment) > 0)
            ...
            if (((LPNMHDR)lParam)->code == NM_CUSTOMDRAW)
            {
                ...
                if (pDraw->nmcd.dwDrawStage == CDDS_ITEMPREPAINT)
                {
                    ...
                    tvi.hItem = (HTREEITEM)pDraw->nmcd.dwItemSpec;
                    char text[512] = {0};
                    tvi.pszText = text;
                    tvi.cchTextMax = sizeof(text)-1;
                    TreeView_GetItem(hTree, &tvi);
                    auto it = addrComments.find((duint)tvi.lParam);
                    if (it != addrComments.end() && !it->second.empty())
                    {
                        // 计算地址部分宽度
                        SIZE sz;
                        ...
                        char addrPart[128] = {0};
                        const char* pSlash = strstr(text, " // ");
                        if (pSlash)
                        {
                            size_t addrLen = pSlash - text;
                            strncpy(addrPart, text, addrLen);
                            addrPart[addrLen] = 0;
                        }
                        else
                        {
                            strcpy(addrPart, text);
                        }
                        GetTextExtentPoint32A(hdc, addrPart, strlen(addrPart), &sz);
                        // 备注绿色显示
                        SetTextColor(hdc, RGB(0, 180, 0));
                        int x = pDraw->nmcd.rc.left + sz.cx + 4;
                        int y = pDraw->nmcd.rc.top;
                        TextOutA(hdc, x, y, it->second.c_str(), (int)it->second.length());
